...
const EmployeeProfileCard = () => {
  ...
  employees.filter(emp => emp.user_type === 'employee')
   .map((emp) => ({
    ...emp,
            ...
            {filteredEmployees.map((emp, idx) => (
              ...
              >
                ...
                <div className="text-[13px] mt-5 space-y-[6px]">
                  <div className="flex justify-between"><span className="text-[#6B7280]">Role:</span><span>{emp.user_role}</span></div>
                  <div className="flex justify-between"><span className="text-[#6B7280]">Department:</span><span>{emp.department}</span></div>
                  ...
                  <div className="mt-4">
                    ...
                    <div className="w-full h-[6px] bg-gray-200 rounded-full">
                      <div className="h-[6px] rounded-full bg-[#EFCD78]" style={{ width: `${emp.attendance}%` }}></div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="flex items-center gap-2 bg-white rounded-[6px] p-[4px] shadow-[0_4px_12px_0_rgba(239,205,120,0.10)]">
          <div
            ...
            className={`flex items-center justify-center rounded-[6px] p-2 cursor-pointer ${
              view === "cards" ? "bg-[#EFCD78] shadow-[0_4px_6px_-4px_rgba(0,0,0,0.10)]" : "bg-white"
            }`}
          >
            <OneColView />
          </div>
          <div
            ...
            className={`flex items-center justify-center rounded-[6px] p-2 cursor-pointer ${
              view === "table" ? "bg-[#EFCD78] shadow-[0_4px_6px_-4px_rgba(0,0,0,0.10)]" : "bg-white"
            }`}
          >
            <RowColView />
          </div>
